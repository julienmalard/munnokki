...
from முன்னோக்கி.காலநிலை.மாறிலிகள் import காலநிலை_மாறி_அச்சு, காலநிலை_மாதிரி_அச்சு, வறலாற்று_காட்சி
...
class செல்ஸா_காலநிலை_வகைகள்(செல்ஸா_உயிரிகாலநிலை):
    def __init__(தன், உறுதி=1, மாறிகள்: Optional[set[str]] = None):
        super().__init__(
            மாறிகள்=மாறிகள்
            or {
                "kg0",
    ...
    def தொலைவு(
        ...
        return xr.where(
            (மூல்_காலநிலை_குறிப்பு == இலக்கு_மாறிகள்).sum(dim=காலநிலை_மாறி_அச்சு)
            / len(தன்.மாதிரிகள்)
            >= தன்.உறுதி,
            1,
            0,
